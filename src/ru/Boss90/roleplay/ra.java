    ...
    public boolean onCommand(CommandSender sender, Command cmd, String str, String[] args) {
        ...
        if (main.SQL.getPlayerRabota(p.getName()).getFraction().getName().contains("Не имеется")) {
            ...
            RaEvents.police.forEach((p2) -> {
                p2.sendMessage(ChatColor.BLUE + "[R] [Полиция] [" + main.SQL.getPlayerRabota(p.getName()).getName() + "] " + p.getName() + ": " + sb.toString());
            });
            return true;
        } else if (main.SQL.getPlayerRabota(sender.getName()).getFraction().equals(Fraction.MERIA)) {
            RaEvents.meria.forEach((p2) -> {
                p2.sendMessage(ChatColor.YELLOW + "[R] [Мэрия] [" + main.SQL.getPlayerRabota(p.getName()).getName() + "] " + p.getName() + ": " + sb.toString());
            });
            return true;
        } else if (main.SQL.getPlayerRabota(sender.getName()).getFraction().equals(Fraction.HOSPITAL)) {
            RaEvents.hospital.forEach((p2) -> {
                p2.sendMessage(ChatColor.RED + "[R] [Больница] [" + main.SQL.getPlayerRabota(p.getName()).getName() + "] " + p.getName() + ": " + sb.toString());
            });
            return true;
        } else {
            return true;
